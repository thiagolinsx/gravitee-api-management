 ...
 */
@ApiDescriptor("/io/gravitee/gateway/standalone/tests/integration/http/redirect.json")
public class FollowRedirectTest extends AbstractWiremockGatewayTest {
    ...
    public void shouldFollowRedirect() throws Exception {
        ...
        wireMockRule.stubFor(WireMock.get("/final").willReturn(WireMock.ok()));

        HttpResponse response = execute(Request.Get("http://localhost:8082/api/redirect")).returnResponse();

        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusLine().getStatusCode());

        wireMockRule.verify(WireMock.getRequestedFor(WireMock.urlPathEqualTo("/redirect")));
        wireMockRule.verify(WireMock.getRequestedFor(WireMock.urlPathEqualTo("/final")));
    }
}
