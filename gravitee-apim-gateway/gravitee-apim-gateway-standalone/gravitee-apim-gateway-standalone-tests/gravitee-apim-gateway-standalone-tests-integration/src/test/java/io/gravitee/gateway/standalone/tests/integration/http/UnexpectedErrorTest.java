...
import io.gravitee.gateway.standalone.tests.integration.policy.UnexpectedErrorPolicy;
 ...
 */
@ApiDescriptor(value = "/io/gravitee/gateway/standalone/tests/integration/http/unexpected-error.json")
public class UnexpectedErrorTest extends AbstractWiremockGatewayTest {
    ...
    public void shouldCancelOnUnexpectedError() throws Exception {
        wireMockRule.stubFor(WireMock.get("/unexpected_error").willReturn(WireMock.ok()));

        final HttpResponse response = execute(Request.Get("http://localhost:8082/test/unexpected_error")).returnResponse();
        Assert.assertEquals(HttpStatusCode.INTERNAL_SERVER_ERROR_500, response.getStatusLine().getStatusCode());
        wireMockRule.verify(WireMock.getRequestedFor(WireMock.urlPathEqualTo("/unexpected_error")));
    }
