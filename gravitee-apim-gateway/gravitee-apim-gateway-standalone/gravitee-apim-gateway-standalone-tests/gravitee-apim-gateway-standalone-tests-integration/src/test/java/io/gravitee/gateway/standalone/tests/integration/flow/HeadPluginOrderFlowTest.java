...
import io.gravitee.gateway.standalone.tests.sdk.flow.policy.Header2Policy;
 ...
 */
@ApiDescriptor("/io/gravitee/gateway/standalone/tests/integration/flow/simple-request-flow.json")
    ...
    public void shouldRunFlows() throws Exception {
        ...
        wireMockRule.verify(
            WireMock
                ...
                .withHeader("X-Gravitee-Policy", WireMock.equalTo("request-header2"))
        );

        Assert.assertEquals(response.getFirstHeader("X-Gravitee-Policy").getValue(), "response-header2");
