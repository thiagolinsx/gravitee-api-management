...
public class ParallelizedRequestsGatewayTest extends AbstractWiremockGatewayTest {
    ...
    public void shouldProcessParallelRequests() throws Exception {
                                ...
                                .send(
                                    httpClientResponseEvt -> {
                                        if (httpClientResponseEvt.succeeded()) {
                                            Assert.assertEquals(HttpStatusCode.OK_200, httpClientResponseEvt.result().statusCode());
        ...
        wireMockRule.verify(NUMBER_OF_CLIENTS * NUMBER_OF_REQUESTS, WireMock.getRequestedFor(WireMock.urlPathEqualTo("/team/my_team")));
    }
}
