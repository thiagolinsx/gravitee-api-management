 ...
 */
@ApiDescriptor("/io/gravitee/gateway/standalone/tests/integration/flow/simple-flow.json")
public class SimpleFlowTest extends AbstractWiremockGatewayTest {
    ...
    public void shouldRunFlows_getMethod() throws Exception {
        ...
        Assert.assertEquals("2", response.getFirstHeader("my-counter").getValue());
        wireMockRule.verify(
            WireMock.getRequestedFor(WireMock.urlPathEqualTo("/team/my_team")).withHeader("my-counter", WireMock.equalTo("1"))
        );
    }

    @Test
    public void shouldNotRunFlows_deleteMethod() throws Exception {
        wireMockRule.stubFor(WireMock.delete("/team/my_team").willReturn(WireMock.ok()));

        final HttpResponse response = execute(Request.Delete("http://localhost:8082/test/my_team")).returnResponse();
        Assert.assertEquals(HttpStatusCode.OK_200, response.getStatusLine().getStatusCode());
        wireMockRule.verify(WireMock.deleteRequestedFor(WireMock.urlPathEqualTo("/team/my_team")).withoutHeader("my-counter"));
    }
