...
import io.gravitee.gateway.standalone.tests.sdk.wiremock.ResourceUtils;
 ...
 */
@ApiDescriptor("/io/gravitee/gateway/standalone/tests/integration/http/client-authentication-pem-support.json")
public class ClientAuthenticationPEMTest extends AbstractWiremockGatewayTest {
    ...
    protected WireMockRule getWiremockRule() {
        return new WireMockRule(
            WireMockConfiguration
                ...
                .dynamicHttpsPort()
                .needClientAuth(true)
                .trustStorePath(ResourceUtils.toPath("io/gravitee/gateway/standalone/tests/integration/truststore01.jks"))
                .trustStorePassword("password")
                .keystorePath(ResourceUtils.toPath("io/gravitee/gateway/standalone/tests/integration/keystore01.jks"))
    ...
    public void simple_request_client_auth() throws Exception {
        ...
        Assert.assertEquals("without ssl configuration => 502", HttpStatus.SC_BAD_GATEWAY, response.getStatusLine().getStatusCode());
        ...
        Assert.assertEquals(
            "trustAll = false, without truststore => 502",
        ...
        response = execute(Request.Get("http://localhost:8082/test/my_team")).returnResponse();
        Assert.assertEquals("trustAll = true, with truststore => 200", HttpStatus.SC_OK, response.getStatusLine().getStatusCode());

        // Fourth call is calling an endpoint where trustAll = true, without truststore => 200
        response = execute(Request.Get("http://localhost:8082/test/my_team")).returnResponse();
        Assert.assertEquals("trustAll = true, without truststore => 200", HttpStatus.SC_OK, response.getStatusLine().getStatusCode());

        // Check that the stub has been successfully invoked by the gateway
        wireMockRule.verify(2, WireMock.getRequestedFor(WireMock.urlPathEqualTo("/team/my_team")));
    }
        ...
        try {
            for (Endpoint endpoint : api.getProxy().getGroups().iterator().next().getEndpoints()) {
                ...
                if (httpEndpoint.getHttpClientSslOptions() != null && httpEndpoint.getHttpClientSslOptions().getKeyStore() != null) {
                    ...
                    keyStore.setCertPath(ResourceUtils.toPath("io/gravitee/gateway/standalone/tests/integration/client-cert.pem"));
                    keyStore.setKeyPath(ResourceUtils.toPath("io/gravitee/gateway/standalone/tests/integration/client-key.pem"));
                }
                if (httpEndpoint.getHttpClientSslOptions() != null && httpEndpoint.getHttpClientSslOptions().getTrustStore() != null) {
                    ...
                    trustStore.setPath(ResourceUtils.toPath("io/gravitee/gateway/standalone/tests/integration/server-cert.pem"));
                }
